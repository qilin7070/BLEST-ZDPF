...
type scheduler struct {
	...
	maxRTT      float64
	lambdaFlag bool
	lambdaUpdatetime time.Time
	waiting uint64
	lambda      int32
}
const lambdaGood int32 = 10
const lambdaBad int32 = 40

func (sch *scheduler) setup() {
	sch.quotas = make(map[protocol.PathID]uint)
	sch.minRTT = 10000000000
	sch.maxRTT = 0
	sch.lambdaFlag = false
	sch.lambdaUpdatetime =  time.Now()
	sch.lambda = 1200
}
...
func (sch *scheduler) selectPathLowLatencyf(s *session, hasRetransmission bool, hasStreamRetransmission bool, fromPth *path) *path {
...
func (sch *scheduler) selectPathLowLatencys(s *session, hasRetransmission bool, hasStreamRetransmission bool, fromPth *path) *path {
...
func (sch *scheduler) srttsCalculation(s *session , pth *path) float64 {
	var estimate float64
	var cwnd = s.pathManager.oliaSenders[pth.pathID].GetCongestionWindow()
	var inf = pth.sentPacketHandler.GetBytesInFlight()
	if inf + protocol.DefaultTCPMSS> cwnd {
		estimate = sch.maxRTT
	}else {
		slope := sch.maxRTT - sch.minRTT
		proportion := inf / cwnd
		estimate = sch.minRTT + slope * float64(proportion)
	}
	return math.Max(estimate, float64(pth.rttStats.SmoothedRTT()))
}
func (sch *scheduler) blestschedEstimateBytes(pth *path , srtts float64 , s *session) float64 {
	var numRtts float64
	var packets float64
	avgRtt := (sch.minRTT + sch.maxRTT)/2
	if avgRtt == 0{
		avgRtt = 1
	}else {
		numRtts = ( srtts / avgRtt )+1
	}
		packets = ((numRtts-1)/2 * float64(protocol.DefaultTCPMSS)+float64(s.pathManager.oliaSenders[pth.pathID].GetCongestionWindow()))*numRtts
	fmt.Println(sch.lambda)
	return packets * float64(sch.lambda)/1000
}
func (sch *scheduler) updateLambda()  {
	if time.Now().Sub(sch.lambdaUpdatetime) < time.Duration(sch.minRTT) {
		return
	}
	if sch.lambdaFlag == true {
				sch.lambda += lambdaBad
	}else {
				sch.lambda -= lambdaGood
	}
	sch.lambdaFlag = false
	if sch.lambda < 1000 {
		sch.lambda = 1000
	}else if sch.lambda > 1300{
		sch.lambda = 1300
	}
	sch.lambdaUpdatetime = time.Now()
}
