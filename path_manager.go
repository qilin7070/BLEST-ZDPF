...
func (pm *pathManager) setup(conn connection) {
	...
	if conn.RemoteAddr() != nil {
		...
		/*remAddr1:=strings.Replace(conn.RemoteAddr().String(),"2","3",1)*/
		if err != nil {
		...
		if remAddr.IP.To4() != nil {
			...
			var remAddr1 net.UDPAddr = net.UDPAddr{}
			ip := net.IP{byte(10),byte(0),byte(3),byte(4)}
			remAddr1.IP=ip
			remAddr1.Port=6121
			pm.remoteAddrs4 = append(pm.remoteAddrs4, remAddr1)
		} else {
